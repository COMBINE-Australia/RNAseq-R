...
glMDPlot(topt, xval="logFC", yval="B", counts=y$counts, anno=fit.cont$genes, groups=group2, samples=labels,
status=summa.fit[,"B.PregVsLac"], id.column="ENTREZID", search.by="SYMBOL",
display.columns=c("ENTREZID", "SYMBOL", "GENENAME"),
html="volcano", folder="volcano", main="B.PregVsLac")
```

This function creates an html page (./volcano/volcano.html) with a volcano plot on the left and a plot showing the log-CPM per sample for a selected gene on the right. A search bar is available to allow gene IDs (gene Symbols in this case) to be searched.
...
```{r}
# I had to modify to get to show up correctly with release version of Glimma
toptreat <- topTreat(fit.treat,coef=1,number=Inf,sort.by="none")
glMDPlot(topt, xval="AveExpr", yval="logFC", counts=y$counts, anno=fit.cont$genes, groups=group2, samples=labels,
status=res.treat[,"B.PregVsLac"], id.column="ENTREZID", search.by="SYMBOL",
display.columns=c("ENTREZID", "SYMBOL", "GENENAME"),
html="MD-Plot", folder="md", main="B.PregVsLac")
```

```{r,echo=FALSE}
# This isn't working
#glMDPlot(fit.treat, counts=y$counts, anno=fit.cont$genes, groups=group2, samples=labels,
#status=res.treat[,"B.PregVsLac"], id.column="ENTREZID",
#search.by="SYMBOL", display.columns=c("ENTREZID", "SYMBOL",
#"GENENAME"), folder="md", main="B.PregVsLac")
```
...
**Solution**
```{r,echo=FALSE}
go_LPregvsLac <- goana(fit.cont,coef="L.PregVsLac",species="Mm",covariate=gene_length)
