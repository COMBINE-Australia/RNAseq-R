...
author: "Belinda Phipson, Anna Trigos, Matt Ritchie, Maria Doyle, Harriet Dashnow, Charity Law"
...
head(gene_length)
# temporary hotfix for goana bug (21/02/18)
egGO2ALLEGS <- getFromNamespace("org.Mm.egGO2ALLEGS", "org.Mm.eg.db")
db_genes <- AnnotationDbi::Lkeys(egGO2ALLEGS)
valid_genes <- fit.cont$genes$ENTREZID %in% db_genes
# Rerun goana with gene length information
go_length <- goana(fit.cont[valid_genes, ],coef="B.PregVsLac",species="Mm",
                   covariate=gene_length[valid_genes])
...
**Solution**
```{r,echo=FALSE}
go_LPregvsLac <- goana(fit.cont[valid_genes, ],coef="L.PregVsLac",species="Mm",
                       covariate=gene_length[valid_genes])
