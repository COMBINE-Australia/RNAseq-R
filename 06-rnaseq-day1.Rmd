...
#knitr::opts_chunk$set(dev="tiff", dpi=300)
```
...
The only annotation we can see is the Entrez Gene ID, which is not very informative. We would like to add some annotation information. There are a number of ways to do this. We will demonstrate how to do this using the [*org.Mm.eg.db* package](https://bioconductor.org/packages/release/data/annotation/html/org.Mm.eg.db.html)
...
Note: Alternatively, the edgeR function `filterByExpr` can be used to perform this method of filtering automatically, as demonstrated in the limma, Glimma and edgeR [workflow article](https://f1000research.com/articles/5-1408/v3).
...
```{r MDSplots-colours, fig.height=5, fig.width=10}
...
```{r MDSplot-corrected, fig.height=5, fig.width=10}
...
If we wanted to add more annotations, for example colours for both cell type and status, we can't easily do that with `heatmap.2`, but we could use the `aheatmap` function from the *NMF* package. We can specify which columns we want to use from sampleinfo in `annCol=` and coloured bars will be added for those.
```{r heatmap-aheatmap}
mypalette <- brewer.pal(11,"RdYlBu")
morecols <- colorRampPalette(mypalette)
...
```{r MDplots-before-TMM, fig.height=5, fig.width=10}
par(mfrow=c(1,2))
...
```{r MDplots-after-TMM, fig.height=5, fig.width=10}
par(mfrow=c(1,2))
plotMD(y,column = 7)
abline(h=0,col="grey")
plotMD(y,column = 11)
...
**Solution**
```{r MDplots-solution, echo=FALSE, fig.height=5, fig.width=10}
par(mfrow=c(1,2))
plotMD(logcounts,column = 11)
abline(h=0,col="grey")
plotMD(y,column = 11)
...
```{r boxplots-beforeVsafter-TMM, fig.height=5, fig.width=10}
par(mfrow=c(1,2))
...
```{r plots-postDE, fig.height=5, fig.width=10}
...
```{r plots-postDE-solution, echo=FALSE, fig.height=5, fig.width=10}
par(mfrow=c(1,2))
...
```{r stripcharts, fig.width=12, fig.height=5}
par(mfrow=c(1,3))
...
**Solution**
```{r stripcharts-solution, echo=FALSE}
# Solution
...
<iframe src="volcano/XY-Plot.html" width="100%" height="1000"></iframe>
...
```{r MDplots-TREAT, fig.height=5,fig.width=10}
...
<iframe src="md/MD-Plot.html" width="100%" height="1000"></iframe>
...
**Solution**
```{r, echo=FALSE}
Mm.H <- readRDS("data/Mm.c2.all.v7.1.entrez.rds")
...
**Solution**
```{r barcodeplots-solution, echo=FALSE}
# Solution 1
